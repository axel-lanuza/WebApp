...
(function () {
    ...
    var CurveLine =
       ...
       BMapLib.CurveLine = function (points, opts) {
           var curvePoints = getCurvePoints(points, opts.strokeCount);
   ...
   * 根据弧线的坐标节点数组
 */
    getCurvePoints = function (points, strokeCount) {
        var curvePoints = [];
        for (var i = 0; i < points.length - 1; i++) {
            var p = getCurveByTwoPoints(points[i], points[i + 1], strokeCount);
            if (p && p.length > 0) {
  ...
  * @param Point 终点
   */
    getCurveByTwoPoints = function (obj1, obj2, strokeCount) {
        ...
        var count = (strokeCount > 0) ? strokeCount : 1; // 曲线是由一些小的线段组成的，这个表示这个曲线所有到的折线的个数
        var isFuture = false;
